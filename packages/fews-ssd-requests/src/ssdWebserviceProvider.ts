...
export class SsdWebserviceProvider {
  ...
  constructor(
    ...
    this.piWebservice = new PiRestService(
      this._piUrl,
      options.transformRequestFn
    );
    this.ssdWebservice = new PiRestService(
      this._ssdUrl,
      options.transformRequestFn
    );
  }
  ...
  public async getSvg(url: string): Promise<SVGElement> {
    ...
    const svgResponse = await this.ssdWebservice.getDataWithParser(
      url,
      requestOptions,
      new SvgElementParser()
    );
   ...
   */
  public async getActionFromElement(
    element: SVGElement,
  ...
  public urlForPanel(panelName: string, date: Date): string {
    ...
    const request =
      "?request=GetDisplay&ssd=" + panelName + "&time=" + dateString;
  ...
  public async getCapabilities(
    ...
    const result =
      await this.ssdWebservice.getDataWithParser<SsdGetCapabilitiesResponse>(
        "?request=GetCapabilities&format=application/json",
        new RequestOptions(),
        parser
      );
    return result.data;
